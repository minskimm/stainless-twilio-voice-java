...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { TwilioVoiceOpenAPIInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw TwilioVoiceOpenAPIInvalidDataException("`$name` is not set")
            is JsonNull -> throw TwilioVoiceOpenAPIInvalidDataException("`$name` is null")
            else -> throw TwilioVoiceOpenAPIInvalidDataException("`$name` is invalid, received $this")
        }
